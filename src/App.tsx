...
function setPageMeta(view: View) {
  ...
  ensureMeta('description', meta.description); ensureMeta('robots', 'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1');
...
export default function App() {
  ...
  </Suspense></main>{view !== 'landing' && <Footer onNavigate={navigate} />}<AuthModal open={authModal.open} mode={authModal.mode} onClose={closeAuth} /><AIProblemSolver /><PWAInstallPrompt />{showOpening && <LogoOpening onDone={finishOpening} />}</div></AppErrorBoundary>;
}