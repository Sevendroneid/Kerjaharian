...
export function JobTimer({ role, lang = 'id' }: JobTimerProps) {
  ...
  let timing: ReturnType<typeof calculateJobTiming> | null = null;
  if (active?.duration_minutes && active.started_at && active.scheduled_end_at) {
    try {
      timing = calculateJobTiming(
        { durationMinutes: active.duration_minutes, basePrice: active.wage ?? 0, overtimeRatePerMinute: active.overtime_rate_per_minute ?? 0, alertBeforeMinutes: 15 },
        { startedAt: active.started_at, scheduledEndAt: active.scheduled_end_at, now },
      );
    } catch (timingError) {
      console.error('Job timer timing error:', timingError);
    }
  }
  ...
  return (
    <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {jobs.map((job) => {
        ...
        const workerAmount = job.worker_amount ?? (job.wage ?? 0) + (job.worker_overtime_amount ?? job.overtime_amount ?? 0);
