...
export function OvertimeApprovalPanel({ role }: { role: 'worker' | 'employer' }) {
  ...
  return <section className="mb-6 rounded-2xl border border-warning-200 bg-white p-5 shadow-sm">
    <div className="flex items-start gap-3"><Clock3 className="mt-0.5 h-5 w-5 text-warning-600"/><div><h2 className="font-display font-bold text-slate-900">Persetujuan Lembur</h2><p className="mt-1 text-xs leading-5 text-slate-500">Lembur tidak aktif tanpa persetujuan eksplisit mitra. Tarif dihitung server berdasarkan tarif pekerjaan.</p></div></div>
    ...
    <div className="mt-4 space-y-3">{visible.map(job => {
      ...
      return <div key={job.id} className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold">{job.title || 'Pekerjaan'}</p><p className="mt-1 text-xs text-slate-500">{expired ? 'Durasi deal telah berakhir.' : `Sisa waktu hingga ${job.scheduled_end_at ? new Date(job.scheduled_end_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-'}`}</p></div><span className="rounded-full bg-warning-50 px-2.5 py-1 text-[10px] font-bold text-warning-700">{job.overtime_consent_status}</span></div>
        {expired && job.overtime_consent_status === 'not_requested' && role === 'employer' && <button disabled={busy===job.id} onClick={() => void actions(job,'request')} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-warning-500 px-3 py-2.5 text-sm font-bold text-white"><AlertTriangle className="h-4 w-4"/>Minta Persetujuan Lembur</button>}
        {expired && job.overtime_consent_status === 'requested' && role === 'worker' && <div className="mt-3 rounded-lg bg-white p-3 ring-1 ring-warning-200"><p className="text-xs font-semibold text-slate-700">Employer meminta Anda melanjutkan pekerjaan sebagai lembur. Persetujuan Anda wajib sebelum lembur dihitung.</p><div className="mt-3 grid grid-cols-2 gap-2"><button disabled={busy===job.id} onClick={() => void actions(job,'decline')} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-bold">Tidak</button><button disabled={busy===job.id} onClick={() => void actions(job,'confirm')} className="rounded-lg bg-warning-500 px-3 py-2 text-sm font-bold text-white">Setujui Lembur</button></div></div>}
        {job.overtime_consent_status === 'confirmed' && <div className="mt-3 rounded-lg bg-warning-50 p-3 text-xs font-semibold text-warning-800"><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4"/>Lembur disetujui mitra.</div><p className="mt-1">{minutes} menit · {formatIDR(amount)} · tarif {formatIDR(Number(job.overtime_rate_per_minute ?? 0))}/menit</p>{role === 'employer' && <button disabled={busy===job.id} onClick={() => void actions(job,'finish')} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-success-600 px-3 py-2.5 text-sm font-bold text-white"><CheckCircle2 className="h-4 w-4"/>Selesai & Tutup Pembayaran</button>}</div>}
        {job.overtime_consent_status === 'declined' && <p className="mt-3 text-xs font-semibold text-slate-500">Mitra tidak menyetujui lembur. Tidak ada biaya lembur yang ditambahkan.</p>}
      </div>;
    })}</div>
  </section>;
}
