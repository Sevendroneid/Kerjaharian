...
export function AIProblemSolver() {
  ...
  const loadProactive = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai-user-proactive', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` }, body: JSON.stringify({ metrics: { open_jobs: openJobs, is_online: profile.is_online } }) });
      const data = await response.json(); if (response.ok) setProactive(data);
  ...
  const ask = async (value = message) => {
    ...
    try { const token = user ? (await supabase.auth.getSession()).data.session?.access_token : null;
      ...
      const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Problem Solver sedang tidak tersedia.'); setAnswer(data); setMessage('');
    } catch (cause) { setError(cause instanceof DOMException && cause.name === 'AbortError' ? 'Permintaan terlalu lama. Coba lagi.' : cause instanceof Error ? cause.message : 'Tidak dapat menghubungi Problem Solver.'); }
  ...
  return <>
    ...
    {open && <div className={`fixed bottom-4 right-4 z-[60] w-[calc(100vw-2rem)] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 sm:bottom-5 sm:right-5 ${showResolution ? 'max-w-lg' : 'max-w-sm'}`} role="dialog" aria-modal="true" aria-label="KerjaHarian AI Problem Solver">
      <div className="flex items-center justify-between bg-slate-950 px-3 py-2.5 text-white"><div className="flex min-w-0 items-center gap-2"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white/10"><Bot className="h-4 w-4" /></div><div className="min-w-0"><p className="truncate text-xs font-extrabold">KerjaHarian</p><p className="truncate text-[10px] text-slate-300">{showResolution ? 'Pusat Resolusi' : 'AI Assistant'}</p></div></div><div className="flex items-center gap-1"><button onClick={() => setShowResolution(v => !v)} className="grid h-9 w-9 place-items-center rounded-lg hover:bg-white/10" aria-label="Pusat Resolusi" title="Pusat Resolusi"><ShieldCheck className="h-4 w-4" /></button><button onClick={closeSolver} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg hover:bg-white/10" aria-label="Tutup AI"><X className="h-4 w-4" /></button></div></div>
      {showResolution ? <div className="max-h-[78vh] overflow-y-auto p-3"><ResolutionCenter /></div> : <>
        <div className="max-h-[60vh] overflow-y-auto p-3">
          {user && proactive && !answer && <div className="mb-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="flex items-center gap-2"><Lightbulb className="h-4 w-4"/><p className="text-xs font-extrabold">AI menyarankan</p></div><p className="mt-2 text-xs font-semibold leading-relaxed text-slate-700">{proactive.summary}</p>{proactive.recommendations?.map((item, i) => <div key={`${item.title}-${i}`} className="mt-3 rounded-lg bg-white p-2.5 ring-1 ring-slate-200"><div className="flex items-center justify-between gap-2"><p className="text-[11px] font-extrabold">{item.title}</p><span className="text-[9px] font-black uppercase text-slate-500">{item.priority}</span></div><p className="mt-1 text-[10px] leading-relaxed text-slate-600">{item.reason}</p><p className="mt-1 text-[10px] font-bold text-slate-700">{item.action}</p></div>)}</div>}
          ...
          {proactiveBusy && !answer && <div className="mb-3 flex items-center gap-2 text-xs font-semibold text-slate-500"><Loader2 className="h-4 w-4 animate-spin"/>Menyiapkan saran untuk Anda…</div>}
          {message && answer && <div className="mb-3 rounded-2xl rounded-br-md bg-slate-100 px-3 py-2 text-xs text-slate-700">{message}</div>}{busy && <div className="mt-3 flex items-center gap-2 text-xs font-semibold text-slate-500"><Loader2 className="h-4 w-4 animate-spin"/>Menganalisis masalah…</div>}{error && <div className="mt-3 rounded-xl bg-red-50 p-3 text-xs font-semibold text-red-700">{error}</div>}
