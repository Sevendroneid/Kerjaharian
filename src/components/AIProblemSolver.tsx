...
export function AIProblemSolver() {
  ...
  const closeSolver = () => setOpen(false);
  ...
  return <>
    ...
    {open && <div className="fixed bottom-4 right-4 z-[60] w-[calc(100vw-2rem)] max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 sm:bottom-5 sm:right-5" role="dialog" aria-modal="true" aria-label="KerjaHarian AI Problem Solver">
      <div className="flex items-center justify-between bg-slate-950 px-3 py-2.5 text-white"><div className="flex min-w-0 items-center gap-2"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white/10"><Bot className="h-4 w-4" /></div><div className="min-w-0"><p className="truncate text-xs font-extrabold">KerjaHarian AI</p><p className="truncate text-[10px] text-slate-300">Problem Solver</p></div></div><button onClick={closeSolver} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg hover:bg-white/10" aria-label="Tutup AI"><X className="h-4 w-4" /></button></div>
