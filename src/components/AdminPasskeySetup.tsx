...
const withTimeout = async <T,>(promise: Promise<T>, ms: number): Promise<T> => {
  ...
  try {
    return await Promise.race([
      promise,
      new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error('Operasi keamanan timeout. Silakan coba lagi.')), ms); })
    ]);
...
export default function AdminPasskeySetup() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { data } = await withTimeout(supabase.auth.passkey.list(), 4000);
        if (active) setReady(Boolean(data?.length));
      } catch (error) {
        // A failed/unsupported listing must never block the Security tab.
  ...
  const register = async () => {
    ...
    try {
      const { data, error } = await withTimeout(supabase.auth.registerPasskey(), 20000);
  ...
  const savePin = async () => {
    ...
    try {
      const { data: current, error: userError } = await withTimeout(supabase.auth.getUser(), 8000);
      if (userError) throw userError;
      if (!current.user?.id) throw new Error('Sesi Admin tidak ditemukan. Silakan login ulang.');
      const { data: profile, error: profileError } = await withTimeout(supabase.from('profiles').select('role').eq('id', current.user.id).maybeSingle(), 8000);
      if (profileError) throw profileError;
      if (profile?.role !== 'admin') throw new Error('Akun aktif bukan akun Admin KerjaHarian.');
      const { error } = await withTimeout(supabase.auth.updateUser({ password: pin }), 10000);
      if (error) throw error;
