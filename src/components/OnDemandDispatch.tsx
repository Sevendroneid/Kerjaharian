...
import { Bell, MapPin, Navigation, Loader2 } from 'lucide-react';
...
export function WorkerDispatchPanel() {
  ...
  return <section className="mb-6 rounded-2xl border-2 border-primary-200 bg-primary-50/60 p-5 shadow-sm" aria-live="polite">
    <div className="flex items-center gap-2"><Bell className="h-5 w-5 text-primary-600" /><div><h2 className="font-display font-bold text-slate-900">Panggilan Kerja Terdekat</h2><p className="text-xs text-slate-500">Dispatch real-time — tawaran berlaku 30 detik.</p></div></div>
...
export function EmployerDispatchWatcher() {
  ...
  useEffect(() => {
    ...
    const dispatch = async () => {
      ...
      for (const row of data ?? []) { if (stopped) return; const { error }=await supabase.rpc('dispatch_open_job',{p_job_id:row.id,p_limit:10}); if(error && !error.message.toLowerCase().includes('koordinat')) console.warn('Dispatch:',error.message); }
    };
    void dispatch(); const timer=window.setInterval(()=>void dispatch(),15000); return()=>{stopped=true;window.clearInterval(timer);};
  },[profile?.role,user]);
  return null;
}
