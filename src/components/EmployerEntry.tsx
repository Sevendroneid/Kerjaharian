...
export function EmployerEntry({onAuthClick,initialCategory,lang,i18n:_i18n}:EmployerEntryProps){
 ...
 return <div className="bg-slate-50 pb-20 pt-6 sm:pt-8"><div className="container-app"><div className="mx-auto max-w-5xl">
  ...
  <section className="mt-4 card p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div><h2 className="font-extrabold text-slate-900">2. {lang==='id'?'Top Up':'Top Up'}</h2><p className="mt-1 text-sm text-slate-500">{lang==='id'?'Isi saldo sebelum memilih dan membuat order.':'Add balance before choosing and creating an order.'}</p></div><Wallet className="h-5 w-5 text-primary-600"/></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-4"><div><p className="text-xs font-bold uppercase text-slate-500">{lang==='id'?'Saldo tersedia':'Available balance'}</p><p className="text-2xl font-extrabold">{formatIDR(wallet?.available_balance??0)}</p></div><button type="button" onClick={()=>setShowOrderForm(true)} className="btn-primary flex items-center gap-2"><PlusCircle className="h-4 w-4"/>{lang==='id'?'Lanjut ke Order':'Continue to Order'}</button></div><p className="mt-2 text-xs text-slate-400">{lang==='id'?'Jika saldo belum cukup, form order akan meminta top-up melalui Midtrans.':'If the balance is insufficient, the order form will request a Midtrans top-up.'}</p></section>
  <section className="mt-4 card p-5 sm:p-6"><h2 className="font-extrabold text-slate-900">3. {lang==='id'?'Pilih / Buat Order':'Choose / Create Order'}</h2><p className="mt-1 text-sm text-slate-500">{lang==='id'?'Pilih kategori pekerjaan, lihat total final, lalu buat order.':'Choose a job category, review the final total, then create the order.'}</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{CATEGORIES.map(category=>{const Icon=category.icon;return <button key={category.id} type="button" onClick={()=>chooseCategory(category.id)} className="group rounded-2xl border-2 border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-primary-400 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"><div className="flex items-start justify-between gap-3"><div className="rounded-xl bg-primary-50 p-3"><Icon className="h-6 w-6 text-primary-600"/></div><ArrowRight className="mt-2 h-5 w-5 text-slate-300 transition group-hover:translate-x-1 group-hover:text-primary-500"/></div><h3 className="mt-4 font-extrabold text-slate-900">{category.label}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{category.examples?.[0]??'Pilih pekerjaan yang Anda butuhkan.'}</p></button>})}</div></section>
 </div></div></div>;
}
