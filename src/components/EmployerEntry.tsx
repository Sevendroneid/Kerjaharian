...
import { ArrowRight, Briefcase, CheckCircle2, Clock3, PlusCircle, ShieldCheck, Wallet, LogIn, UserPlus } from 'lucide-react';
...
export function EmployerEntry({onAuthClick,initialCategory,lang,i18n:_i18n}:EmployerEntryProps){
 ...
 const startTopup=()=>{if(!user){onAuthClick('signup');return}setOpenTopup(true);setShowOrderForm(true)};
 return <div className="bg-slate-50 pb-20 pt-6 sm:pt-8"><div className="container-app"><div className="mx-auto max-w-5xl">
  <div className="mb-5 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-7">
   <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    ...
    <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{lang==='id'?'Setelah KYC disetujui, urutan kerja Anda adalah: cek order → top up → pilih dan buat order.':'After KYC approval: check orders → top up → choose and create an order.'}</p></div>
    {!user&&<div className="flex shrink-0 flex-col gap-2 sm:min-w-[180px]">
