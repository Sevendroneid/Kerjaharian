...
import { CheckCircle2, FileCheck2, IdCard, Loader2, Lock, ShieldCheck, Upload, UserRound, XCircle } from 'lucide-react';
...
export default function KycGate({ children }: { children: ReactNode }) {
  ...
  const selectImage = (next: File | undefined, kind: 'ktp' | 'selfie') => {
    if (!next) return;
    ...
    reader.onload = (e) => {
      ...
      if (kind === 'ktp') { setKtpFile(next); setKtpPreview(preview); }
      else { setSelfieFile(next); setSelfiePreview(preview); }
    };
    reader.readAsDataURL(next);
  };

  const submit = async () => {
    if (!ktpFile || !user || (role === 'employer' && !selfieFile)) return;
    setBusy(true); setError('');
    const uploadOne = async (file: File, prefix: string) => {
      const ext = file.type === 'image/png' ? 'png' : file.type === 'image/webp' ? 'webp' : 'jpg';
      const path = `${user.id}/${prefix}-${Date.now()}.${ext}`;
      const result = await supabase.storage.from('kyc-docs').upload(path, file, { contentType: file.type, upsert: false });
    ...
    try {
      ...
      if (role === 'employer' && selfieFile) selfiePath = await uploadOne(selfieFile, 'selfie');
      const update = await supabase.from('profiles').update({ ktp_photo_url: ktpPath, ...(role === 'employer' ? { selfie_photo_url: selfiePath } : {}) }).eq('id', user.id);
      ...
      setBusy(false); setKtpFile(null); setKtpPreview(null); setSelfieFile(null); setSelfiePreview(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal mengirim dokumen verifikasi.');
      setBusy(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto max-w-lg">
        <div className="card overflow-hidden">
          ...
          <div className="p-6">
            {status === 'pending' ? <div className="rounded-xl bg-amber-50 p-5 ring-1 ring-amber-200"><CheckCircle2 className="h-8 w-8 text-amber-600" /><h2 className="mt-3 font-bold text-amber-900">Dokumen sedang diperiksa</h2><p className="mt-1 text-sm leading-relaxed text-amber-800">Dokumen Anda sudah diterima dan menunggu pemeriksaan admin. Anda belum dapat melakukan transaksi sampai identitas disetujui.</p></div> : status === 'rejected' ? <div className="mb-5 rounded-xl bg-red-50 p-4 ring-1 ring-red-200"><XCircle className="h-6 w-6 text-red-600" /><p className="mt-2 text-sm font-bold text-red-900">Verifikasi ditolak</p><p className="mt-1 text-sm text-red-800">{profile.kyc_rejection_reason || 'Dokumen perlu diperbaiki atau diganti.'}</p></div> : null}
            {status !== 'pending' && <>
              <p className="text-sm leading-relaxed text-slate-600">Upload <b>foto KTP asli</b> yang jelas. {role === 'employer' ? <>Untuk Employer, kami juga membutuhkan <b>selfie wajah terbaru</b> untuk membantu Admin memastikan orang pada akun sesuai dengan pemilik KTP.</> : <>Pemeriksaan akhir dilakukan admin KerjaHarian.</>}</p>
              <div className="mt-5 space-y-4">
                <div className="rounded-xl border-2 border-dashed border-slate-200 p-5 text-center">
                  {ktpPreview ? <img src={ktpPreview} alt="Preview KTP" className="mx-auto max-h-56 rounded-lg object-contain" /> : <><IdCard className="mx-auto h-12 w-12 text-slate-300" /><p className="mt-2 text-sm font-semibold text-slate-600">Foto KTP bagian depan</p></>}
                  <input ref={ktpInput} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="hidden" onChange={(e) => selectImage(e.target.files?.[0], 'ktp')} />
                  <button onClick={() => ktpInput.current?.click()} className="btn-secondary mt-4 inline-flex items-center gap-2"><Upload className="h-4 w-4" />{ktpFile ? 'Ganti Foto KTP' : 'Ambil / Pilih Foto KTP'}</button>
                </div>
                {role === 'employer' && <div className="rounded-xl border-2 border-dashed border-slate-200 p-5 text-center">
                  {selfiePreview ? <img src={selfiePreview} alt="Preview selfie" className="mx-auto max-h-56 rounded-lg object-contain" /> : <><UserRound className="mx-auto h-12 w-12 text-slate-300" /><p className="mt-2 text-sm font-semibold text-slate-600">Selfie wajah terbaru</p><p className="mt-1 text-xs text-slate-500">Pastikan wajah terlihat jelas, tanpa masker/kacamata gelap.</p></>}
                  <input ref={selfieInput} type="file" accept="image/jpeg,image/png,image/webp" capture="user" className="hidden" onChange={(e) => selectImage(e.target.files?.[0], 'selfie')} />
                  <button onClick={() => selfieInput.current?.click()} className="btn-secondary mt-4 inline-flex items-center gap-2"><UserRound className="h-4 w-4" />{selfieFile ? 'Ganti Selfie' : 'Ambil Selfie'}</button>
                </div>}
              </div>
              {error && <p className="mt-3 text-sm font-semibold text-red-600">{error}</p>}
              <button disabled={!ktpFile || busy || (role === 'employer' && !selfieFile)} onClick={() => void submit()} className="btn-primary mt-5 flex w-full items-center justify-center gap-2 disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileCheck2 className="h-4 w-4" />}{busy ? 'Mengirim...' : role === 'employer' ? 'Kirim KTP + Selfie untuk Verifikasi' : 'Kirim untuk Verifikasi'}</button>
