...
import { Camera, CheckCircle2, IdCard, Loader2, Lock, ShieldCheck, Upload, UserRound, XCircle } from 'lucide-react';
...
export default function KycGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadLegal = async () => {
      ...
      const { data, error: e } = await supabase.from('platform_consents')
        .select('consent_type,document_version').eq('user_id', user.id)
  ...
  const captureSelfie = () => {
    ...
    canvas.toBlob(blob => {
      ...
      setSelfieFile(file); setSelfiePreview(canvas.toDataURL('image/jpeg', 0.9)); setCameraError(''); closeCamera();
      setStep('face');
    }, 'image/jpeg', 0.9);
  };
      ...
      <div className="p-6">
        ...
        {status !== 'pending' && step === 'identity' && <>
          ...
          <div className="mt-5 rounded-xl border-2 border-dashed border-slate-200 p-5 text-center">{ktpPreview ? <img src={ktpPreview} alt="Preview KTP" className="mx-auto max-h-56 rounded-lg object-contain"/> : <><IdCard className="mx-auto h-12 w-12 text-slate-300"/><p className="mt-2 text-sm font-semibold text-slate-600">Foto KTP bagian depan</p></>}
            <input ref={ktpInput} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="hidden" onChange={e=>selectKtp(e.target.files?.[0])}/><button onClick={()=>ktpInput.current?.click()} className="btn-secondary mt-4 inline-flex items-center gap-2"><Upload className="h-4 w-4"/>{ktpFile ? 'Ganti Foto KTP' : 'Ambil Foto KTP'}</button>
          </div>
          {error && <p className="mt-3 text-sm font-semibold text-red-600">{error}</p>}
          <button disabled={!ktpFile} onClick={()=>setStep(needsFace ? 'face' : 'review')} className="btn-primary mt-5 w-full disabled:opacity-50">{needsFace ? 'Lanjut ke Verifikasi Wajah' : 'Periksa & Kirim'}</button>
        </>}

        {status !== 'pending' && step === 'face' && needsFace && <>
          <div className="rounded-xl bg-primary-50 p-4 ring-1 ring-primary-100"><div className="flex gap-3"><UserRound className="h-5 w-5 shrink-0 text-primary-700"/><div><p className="text-sm font-bold text-primary-900">Langkah 2 — Verifikasi Wajah</p><p className="mt-1 text-xs leading-5 text-primary-800">Seperti onboarding mitra Gojek: gunakan kamera depan secara langsung. Wajah harus jelas dan seluruh wajah terlihat.</p></div></div></div>
          <ul className="mt-4 space-y-2 text-sm text-slate-700"><li>• Lepaskan masker, topi, dan kacamata hitam.</li><li>• Cari tempat terang dan hadapkan wajah ke kamera.</li><li>• Jangan gunakan foto dari galeri atau foto orang lain.</li><li>• Pastikan hanya satu wajah yang terlihat.</li></ul>
          <div className="mt-5 rounded-xl border-2 border-dashed border-slate-200 p-5 text-center">{selfiePreview ? <img src={selfiePreview} alt="Preview selfie" className="mx-auto max-h-64 rounded-xl object-contain"/> : <div className="mx-auto grid h-28 w-28 place-items-center rounded-full bg-slate-100"><Camera className="h-12 w-12 text-slate-300"/></div>}
            <p className="mt-3 text-sm font-semibold text-slate-700">{selfieFile ? 'Selfie berhasil diambil' : 'Belum ada selfie'}</p>
            {!selfieFile && <button onClick={()=>void startCamera()} className="btn-primary mt-4 inline-flex items-center gap-2"><Camera className="h-4 w-4"/>Buka Kamera &amp; Selfie</button>}
          </div>
          {cameraError && <p className="mt-3 rounded-lg bg-red-50 p-3 text-sm font-semibold text-red-700">{cameraError}</p>}
          {error && <p className="mt-3 text-sm font-semibold text-red-600">{error}</p>}
          {selfieFile && <div className="mt-4 grid grid-cols-2 gap-3"><button onClick={()=>{setSelfieFile(null);setSelfiePreview(null);}} className="btn-secondary">Ambil Ulang</button><button onClick={()=>setStep('review')} className="btn-primary">Lanjut ke Review</button></div>}
        </>}

        {status !== 'pending' && step === 'review' && <>
          <div className="rounded-xl bg-emerald-50 p-4 ring-1 ring-emerald-200"><div className="flex gap-3"><CheckCircle2 className="h-5 w-5 shrink-0 text-emerald-700"/><div><p className="text-sm font-bold text-emerald-900">Langkah terakhir — Periksa data</p><p className="mt-1 text-xs leading-5 text-emerald-800">Pastikan foto KTP dan selfie jelas sebelum dikirim. Setelah dikirim, status masuk pemeriksaan admin.</p></div></div></div>
          <div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 p-3"><p className="mb-2 text-xs font-bold text-slate-500">KTP</p>{ktpPreview ? <img src={ktpPreview} alt="KTP" className="h-36 w-full rounded-lg bg-slate-50 object-contain"/> : <p className="text-sm text-red-600">Belum ada KTP</p>}<button onClick={()=>setStep('identity')} className="mt-2 text-xs font-bold text-primary-700">Ganti KTP</button></div>{needsFace && <div className="rounded-xl border border-slate-200 p-3"><p className="mb-2 text-xs font-bold text-slate-500">SELFIE</p>{selfiePreview ? <img src={selfiePreview} alt="Selfie" className="h-36 w-full rounded-lg bg-slate-50 object-contain"/> : <p className="text-sm text-red-600">Belum ada selfie</p>}<button onClick={()=>setStep('face')} className="mt-2 text-xs font-bold text-primary-700">Ambil Ulang</button></div>}</div>
          {error && <p className="mt-3 text-sm font-semibold text-red-600">{error}</p>}
          <button disabled={busy || !ktpFile || (needsFace && !selfieFile)} onClick={()=>void submit()} className="btn-primary mt-5 flex w-full items-center justify-center gap-2 disabled:opacity-50">{busy ? <><Loader2 className="h-4 w-4 animate-spin"/>Mengirim untuk pemeriksaan...</> : 'Kirim untuk Verifikasi'}</button>
        </>}
  ...
  {cameraOpen && <div className="fixed inset-0 z-[200] grid place-items-center bg-slate-950/80 p-4"><div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between p-4"><div><h2 className="font-extrabold text-slate-900">Verifikasi wajah</h2><p className="text-xs text-slate-500">Pastikan wajah terlihat jelas di kamera.</p></div><button onClick={closeCamera} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Tutup kamera">×</button></div><div className="relative bg-black"><video ref={videoRef} muted playsInline autoPlay className="aspect-square w-full object-cover"/><div className="pointer-events-none absolute inset-0 grid place-items-center"><div className="h-64 w-52 rounded-[45%] border-2 border-white/80"/></div>{cameraLoading && <div className="absolute inset-0 grid place-items-center bg-black/50 text-white"><Loader2 className="h-8 w-8 animate-spin"/></div>}</div><div className="space-y-3 p-4"><p className="text-xs leading-5 text-slate-600">Wajah harus terang, tidak tertutup masker/topi/kacamata hitam, dan hanya satu orang di dalam frame.</p>{cameraError && <p className="rounded-lg bg-red-50 p-3 text-xs font-semibold text-red-700">{cameraError}</p>}<button disabled={cameraLoading || !!cameraError} onClick={captureSelfie} className="btn-primary flex w-full items-center justify-center gap-2 disabled:opacity-50"><Camera className="h-4 w-4"/>Ambil Selfie Sekarang</button></div></div></div>}
  </div>;
}
