...
import { Menu, X, LogOut, Loader2, MessageCircle, Wallet, ArrowDownToLine, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export function Header({view,onNavigate,onAuthClick,lang,onLangChange}:HeaderProps){
  ...
  const openKyc=()=>{window.location.href='/verifikasi-kyc';setMobileOpen(false)};
  ...
  return <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled?'bg-white/85 backdrop-blur-xl shadow-soft ring-1 ring-slate-200/60':'bg-transparent'}`}><div className="container-app"><div className="flex h-16 items-center justify-between">
    ...
    <div className="hidden items-center gap-2 md:flex">
      {user&&profile?.role==='employer'&&!profile?.kyc_verified&&<button type="button" onClick={openKyc} className="flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-extrabold text-white shadow-sm hover:bg-amber-600"><ShieldCheck className="h-3.5 w-3.5"/>{lang==='id'?'Daftar KYC':'Register KYC'}</button>}
