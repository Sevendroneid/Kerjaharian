  ...
  const checkRecoveryStatus = useCallback(async () => {
    ...
    try {
      ...
      if (status?.status === 'success' && status?.action_link) {
        ...
        window.location.assign(status.action_link);
        return true;
      }
