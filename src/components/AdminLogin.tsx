...
const ADMIN_PHONE = '082340871029';
...
export default function AdminLogin({ onClose }: AdminLoginProps) {
  ...
  const handlePin = async () => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('admin-pin-auth', { body: { phone: ADMIN_PHONE, pin } });
      if (fnError) throw fnError;
      if (!data?.success || !data?.action_link) throw new Error(data?.error || 'Login admin gagal');
  ...
  const handlePasskey = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Passkey belum terdaftar di perangkat ini.');
    } finally { setLoading(false); }
  };

  return <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
      <div className="flex items-center gap-3 mb-5"><div className="rounded-xl bg-slate-900 text-white p-2.5"><ShieldCheck size={22}/></div><div><h2 className="text-xl font-bold">Login Admin</h2><p className="text-xs text-slate-500">Akun khusus {ADMIN_PHONE}</p></div></div>
      <label className="text-xs font-semibold text-slate-600">PIN Admin</label>
      <input value={pin} onChange={e => setPin(e.target.value.replace(/\D/g,'').slice(0,8))} type="password" inputMode="numeric" autoComplete="one-time-code" maxLength={8} placeholder="8 digit PIN" className="w-full border rounded-xl px-4 py-3 mt-1 text-center text-lg tracking-[0.35em]" onKeyDown={e => { if (e.key === 'Enter') void handlePin(); }} />
