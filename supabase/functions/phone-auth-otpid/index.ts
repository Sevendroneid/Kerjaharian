...
function validPhone(phone: string) { return /^628\d{8,11}$/.test(phone); }

async function requestInbound(phone: string) {
  const response = await fetch(`${OTPID_BASE_URL}/v3/request`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${OTPID_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ channel: 'whatsapp_inbound', destination: phone, brand: 'KerjaHarian', ttl: 300 }),
  });
...
async function findOrCreateUser(phone: string) {
  const { data: profile } = await supabaseAdmin.from('profiles').select('id').or(`phone.eq.${phone},whatsapp.eq.${phone},phone.eq.+${phone},whatsapp.eq.+${phone}`).limit(1).maybeSingle();
  if (profile?.id) {
  ...
  const { data: created, error: createError } = await supabaseAdmin.auth.admin.createUser({ phone: `+${phone}`, phone_confirm: true, email, email_confirm: true, user_metadata: { phone_auth: true, phone } });
...
async function createSessionLink(phone: string) {
  ...
  if (!email) {
    ...
    const { data: updated, error } = await supabaseAdmin.auth.admin.updateUserById(user.id, { email, email_confirm: true });
    if (error || !updated.user) throw error || new Error('Gagal menyiapkan sesi login');
  }
  const { data, error } = await supabaseAdmin.auth.admin.generateLink({ type: 'magiclink', email, options: { redirectTo: SITE_URL } });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'request') {
      ...
      const { data: recentChallenge, error: recentError } = await supabaseAdmin.from('phone_auth_otpid_challenges').select('id').eq('phone', phone).is('consumed_at', null).gt('created_at', cutoff).limit(1).maybeSingle();
      ...
      const { data: challenge, error } = await supabaseAdmin.from('phone_auth_otpid_challenges').insert({ phone, otp_id: data.otp_id, expires_at: expiresAt }).select('id, expires_at').single();
      if (error) throw error;
