...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!/^62\d{8,15}$/.test(target)) return new Response(JSON.stringify({ error:'Nomor WhatsApp tidak valid' }), { status:400, headers });
    const { data: recent } = await supabase.from('otp_codes').select('created_at').eq('phone',target).gt('created_at',new Date(Date.now()-60_000).toISOString()).limit(1);
    if (recent?.length) return new Response(JSON.stringify({ error:'Tunggu 60 detik sebelum meminta OTP lagi', retry_after:60 }), { status:429, headers });
    const { count } = await supabase.from('otp_codes').select('id',{count:'exact',head:true}).eq('phone',target).gt('created_at',new Date(Date.now()-24*60*60*1000).toISOString());
    if ((count ?? 0) >= 10) return new Response(JSON.stringify({ error:'Batas permintaan OTP harian tercapai. Coba lagi besok.' }), { status:429, headers });
    ...
    await supabase.from('otp_codes').update({ provider_status:providerOk?'accepted':'failed', provider_detail:JSON.stringify(result).slice(0,1000), sent_at:providerOk?new Date().toISOString():null }).eq('id',row.id);
    if (!providerOk) return new Response(JSON.stringify({ error:'WhatsApp OTP sedang tidak tersedia', detail:result?.reason || result?.detail || 'Fonnte gagal menerima pesan' }), { status:503, headers });
