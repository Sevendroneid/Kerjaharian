...
BEGIN
  ...
  IF TG_OP='INSERT' THEN
    NEW.kyc_verified:=false; NEW.kyc_status:='not_started'; NEW.kyc_submitted_at:=NULL; NEW.kyc_reviewed_at:=NULL; NEW.kyc_reviewed_by:=NULL; NEW.kyc_rejection_reason:=NULL;
  ELSE
    NEW.kyc_verified:=OLD.kyc_verified; NEW.kyc_status:=OLD.kyc_status; NEW.kyc_submitted_at:=OLD.kyc_submitted_at; NEW.kyc_reviewed_at:=OLD.kyc_reviewed_at; NEW.kyc_reviewed_by:=OLD.kyc_reviewed_by; NEW.kyc_rejection_reason:=OLD.kyc_rejection_reason;
  END IF;
...
CREATE FUNCTION public.verify_kyc() RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE v_user uuid:=auth.uid(); v_has_document boolean;
BEGIN
  IF v_user IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  SELECT (ktp_photo_url IS NOT NULL AND ktp_photo_url<>'') INTO v_has_document FROM public.profiles WHERE id=v_user;
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM public.profiles p WHERE p.id=auth.uid() AND p.role='admin') THEN RAISE EXCEPTION 'Admin access required'; END IF;
  IF NOT EXISTS(SELECT 1 FROM public.profiles p WHERE p.id=p_user_id AND p.role IN('worker','employer') AND p.ktp_photo_url IS NOT NULL) THEN RAISE EXCEPTION 'KYC submission not found'; END IF;
  v_status:=CASE WHEN p_approve THEN 'approved' ELSE 'rejected' END;
  UPDATE public.profiles SET kyc_verified=p_approve,kyc_status=v_status,kyc_reviewed_at=now(),kyc_reviewed_by=auth.uid(),kyc_rejection_reason=CASE WHEN p_approve THEN NULL ELSE NULLIF(trim(p_rejection_reason),'') END WHERE id=p_user_id;
  RETURN jsonb_build_object('status',v_status,'user_id',p_user_id);
END;
$$;
REVOKE EXECUTE ON FUNCTION public.admin_review_kyc(uuid,boolean,text) FROM PUBLIC,anon,authenticated; GRANT EXECUTE ON FUNCTION public.admin_review_kyc(uuid,boolean,text) TO authenticated;
