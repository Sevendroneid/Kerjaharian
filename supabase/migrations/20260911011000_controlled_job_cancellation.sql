...
BEGIN
  ...
  IF auth.uid()=v_job.worker_id THEN
    v_actor := 'worker';
    v_event := 'worker_late_cancel';
  ELSIF auth.uid()=v_job.employer_id THEN
    v_actor := 'employer';
    v_event := CASE WHEN v_job.worker_checked_in_at IS NOT NULL THEN 'employer_cancel_after_arrival' ELSE 'employer_cancel_after_accept' END;
  ELSIF NOT is_admin() THEN
    RAISE EXCEPTION 'Tidak berwenang membatalkan pekerjaan ini';
  ELSE
    v_actor := 'admin';
    v_event := 'employer_cancel_after_accept';
  END IF;
  ...
  VALUES(v_job.id,v_job.worker_id,v_job.employer_id,auth.uid(),v_event,CASE WHEN v_event='employer_cancel_after_arrival' THEN 3 ELSE 2 END,jsonb_build_object('reason',trim(p_reason),'actor',v_actor));
