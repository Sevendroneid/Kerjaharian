...
-- Fix employer check-in distance calculation.
-- The longitude delta must compare p_lng with the stored longitude (v_lng),
-- not with the stored latitude (v_lat).
...
BEGIN
  SELECT * INTO v_job
  FROM public.jobs
  WHERE id=p_job_id AND employer_id=auth.uid() AND status='assigned'
  FOR UPDATE;
  ...
  IF v_distance > v_job.presence_radius_meters THEN
    RAISE EXCEPTION 'Anda masih %.0f meter dari lokasi pekerjaan. Maksimal %s meter',v_distance,v_job.presence_radius_meters;
  END IF;

  UPDATE public.jobs SET
    employer_checked_in_at=now(),
    employer_checkin_lat=p_lat,
    employer_checkin_lng=p_lng,
    employer_checkin_photo_path=NULLIF(p_photo_path,''),
    workflow_status=CASE WHEN worker_checked_in_at IS NOT NULL THEN 'ready_to_start' ELSE 'employer_checked_in' END,
    updated_at=now()
  WHERE id=p_job_id
  ...
  VALUES (
    ...
    auth.uid(),
    'EMPLOYER_CHECKED_IN',
    jsonb_build_object('distance_meters',round(v_distance::numeric,1),'latitude',p_lat,'longitude',p_lng,'photo',p_photo_path IS NOT NULL)
  );
