...
DECLARE v_job public.jobs; v_lat numeric; v_lng numeric; v_radius_meters numeric:=10000; v_count integer:=0; v_limit integer;
BEGIN
  ...
  SELECT v_job.id,ranked.id,row_number() OVER (ORDER BY ranked.dispatch_score DESC,ranked.distance_meters),ranked.distance_meters,ranked.dispatch_score
  FROM (
    SELECT p.id,p.rating,
      (6371000*2*asin(sqrt(power(sin(radians(wl.lat-v_lat)/2),2)+cos(radians(v_lat))*cos(radians(wl.lat))*power(sin(radians(wl.lng-v_lng)/2),2)))) AS distance_meters,
      GREATEST(0,COALESCE(p.rating,0))-LEAST(2.0,COALESCE(rel.no_show_count,0)*0.25+COALESCE(rel.cancel_count,0)*0.15)+CASE WHEN COALESCE(rel.completed_count,0)>0 THEN LEAST(1.0,rel.completed_count*0.02) ELSE 0 END AS dispatch_score
    FROM public.profiles p JOIN public.worker_locations wl ON wl.worker_id=p.id
    LEFT JOIN LATERAL (
      SELECT COUNT(*) FILTER (WHERE event_type='worker_no_show')::numeric AS no_show_count,
             COUNT(*) FILTER (WHERE event_type='worker_late_cancel')::numeric AS cancel_count,
             COUNT(*) FILTER (WHERE event_type='overtime_accepted')::numeric AS completed_count
      FROM public.job_reliability_events r WHERE r.worker_id=p.id AND r.created_at>now()-interval '90 days'
    ) rel ON true
    WHERE p.role='worker' AND p.is_online=true AND p.kyc_verified=true AND wl.updated_at>now()-interval '5 minutes'
  ...
  ORDER BY ranked.dispatch_score DESC,ranked.distance_meters LIMIT v_limit;
  GET DIAGNOSTICS v_count=ROW_COUNT;
  INSERT INTO public.job_events(job_id,order_id,actor_id,event_type,metadata) VALUES(v_job.id,v_job.order_id,auth.uid(),'DISPATCH_CREATED',jsonb_build_object('offers',v_count,'radius_meters',v_radius_meters,'ranking','rating_plus_reliability_signal'));
  RETURN v_count;
END;
$function$;
