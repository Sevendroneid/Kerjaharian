...
export async function onRequest(context){
  ...
  try{const a=await auth(request,env);if(!a.ok){safeLog('request_auth_failed',{request_id:requestId,stage:a.stage,status:a.status??null,role:a.role??null});return json({error:'Akses hanya untuk admin yang terautentikasi.',debug_stage:a.stage,debug_status:a.status??null,debug_role:a.role??null,request_id:requestId},401,{...headers,'x-kh-request-id':requestId});}
