...
async function writeAudit(auth, { requestText, action, offerId, jobId, result }) {
  const { error } = await auth.client.from('ai_action_logs').insert({
    ...
    recommendation: { source: 'ai-user', requires_confirmation: true },
    result: result ?? {}
  });
  return !error;
}
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (!auth) return json({ error: 'Sesi worker tidak valid.' }, 401, headers);
    const body = await request.json();
    const action = typeof body?.action === 'string' ? body.action : '';
    if (action !== 'accept_offer') return json({ error: 'Perintah aman yang tersedia saat ini hanya menerima penawaran worker.' }, 403, headers);
    ...
    const auditLogged = await writeAudit(auth, { requestText, action, offerId, jobId: offer.job_id, result: data });
    return json({ ok: true, action, result: data, permission_level: 'confirmation', critical: false, writes_performed: true, audit_logged: auditLogged }, 200, headers);
