import { authenticate, adminContext, clean, classify, cors, json, model, userContext } from '../lib/ai-core.js';
...
function fallback(message, ctx) {
  ...
  if (intent === 'worker_search') return { answer: 'Saya dapat membantu employer memahami kebutuhan dan memantau pencarian pekerja. Untuk keselamatan dan keadilan, kandidat tetap ditentukan oleh matching engine berdasarkan persyaratan pekerjaan dan sinyal trust yang diperbolehkan.', commands: [{ id: 'open_matching', label: 'Buka Cari Pekerja', type: 'navigate', target: '/cari-pekerja' }] };
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const message = clean(body?.message).slice(0, 1600);
    if (!message) return json({ error: 'Pesan wajib diisi.' }, 400, headers);
    const context = await userContext(auth, env);
    const base = fallback(message, context);
    const ai = await model(message, { role: context.role, is_online: context.is_online, jobs: context.jobs.slice(0, 10), own_jobs: context.own_jobs }, env, 'Anda adalah AI User KerjaHarian untuk WORKER dan EMPLOYER. Anda mempunyai kemampuan yang sama secara prinsip: memahami registrasi, pekerjaan, order, matching, harga, masalah dan solusi. Bedakan hanya role dan kewenangan. Berikan solusi konkret berdasarkan CONTEXT. Jangan mengarang. Jangan meminta OTP. Jangan memberikan instruksi diskriminatif. Jangan melakukan critical action. Jika menyarankan tindakan, keluarkan sebagai command aman yang dapat dikonfirmasi UI.');
