...
export async function onRequest(context) {
  ...
  if (job.order_id) {
    const { data: openCases, error: resolutionError } = await admin.from('resolutions').select('id')
      .eq('order_id', job.order_id)
      .in('status', ['open', 'waiting_response', 'under_review', 'waiting_evidence', 'negotiation']).limit(1);
  ...
  const { data: paidRows, error: paidError } = await admin.from('job_payment_events').select('gross_amount').eq('job_id', job.id).eq('payment_status', 'settled');
