...
export async function onRequest(context) {
  ...
  const { data: job, error: findError } = await admin.from('jobs').select('id, final_amount, employer_total, total, payment_status, midtrans_transaction_status, midtrans_transaction_id, paid_at, midtrans_pending_amount').eq('midtrans_order_id', orderId).maybeSingle();
